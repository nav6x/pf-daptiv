...
- Conv1D Layer 1: 32 filters, kernel size $k = 3$, stride $s = 1$, padding same. Uses ReLU activation: $\text{ReLU}(z) = \max(0, z)$.
...
- Conv1D Layer 2: 64 filters, kernel size $k = 3$, stride $s = 1$, padding same. Uses ReLU activation.
- Batch Normalization 2: Re-centers feature distributions across the 64 channels.
- MaxPool1D Layer 2: Window size 2, stride 2. Downsamples from 17 to 8 positions, yielding a tensor of shape $(8, 64)$.

### Stage 3: Dense Representation and Regularization
- Flatten: Unrolls the $(8, 64)$ feature tensor into a 1D vector of length $8 \times 64 = 512$.
- Fully Connected Dense Layer: Projects the 512 features into 128 hidden representations using ReLU activation.
- Dropout Regularization: During training, randomly sets 30 percent of activations to zero ($p = 0.3$). This prevents the network from relying on any single feature subset, preventing overfitting.

### Stage 4: Multiclass Classification Output
- Fully Connected Output Layer: Projects 128 hidden units to 6 logits $z_0, z_1, \dots, z_5$, corresponding to the 6 APT stages.
- Softmax Activation: Converts raw logits into normalized probabilities:

$$
\hat{y}_c = \frac{e^{z_c}}{\sum_{j=0}^5 e^{z_j}}, \quad c \in \{0, 1, \dots, 5\}
$$
...
| Conv1D Layer 1 | $(35, 32)$ | $(3 \times 1 + 1) \times 32 = 128$ | 4.5 KB |
| Batch Normalization 1 | $(35, 32)$ | $2 \times 32 = 64$ | 0.5 KB |
| MaxPool1D 1 | $(17, 32)$ | 0 | 2.2 KB |
| Conv1D Layer 2 | $(17, 64)$ | $(3 \times 32 + 1) \times 64 = 6{,}208$ | 4.3 KB |
| Batch Normalization 2 | $(17, 64)$ | $2 \times 64 = 128$ | 0.5 KB |
| MaxPool1D 2 | $(8, 64)$ | 0 | 2.0 KB |
| Flatten | $(512)$ | 0 | 2.0 KB |
| Fully Connected (128) | $(128)$ | $(512 + 1) \times 128 = 65{,}664$ | 0.5 KB |
| Fully Connected (6) | $(6)$ | $(128 + 1) \times 6 = 774$ | Negligible |
| **Total Model** | **Parameters: 72,966** | **Storage: ~292 KB** | **Runtime RAM: < 2 MB** |

Because the model requires under 300 kilobytes of storage and less than 2 megabytes of RAM, it runs efficiently on standard industrial edge gateways without requiring specialized GPU accelerators.
...
$$
\mathcal{L}_{\text{CE}}(\theta) = - \frac{1}{|D_k|} \sum_{(\mathbf{x}, \mathbf{y}) \in D_k} \sum_{c=0}^5 y_c \ln \hat{y}_c
$$
